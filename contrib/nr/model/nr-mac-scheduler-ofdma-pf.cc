...
TypeId NrMacSchedulerOfdmaPF::GetTypeId (void)
{
...
void NrMacSchedulerOfdmaPF::SetFairnessIndex (double v)
{
  NS_LOG_FUNCTION (this);
  m_alpha = v;
}

double NrMacSchedulerOfdmaPF::GetFairnessIndex () const
{
  NS_LOG_FUNCTION (this);
  return m_alpha;
}

void NrMacSchedulerOfdmaPF::SetTimeWindow (double v)
{
  NS_LOG_FUNCTION (this);
  m_timeWindow = v;
}

double NrMacSchedulerOfdmaPF::GetTimeWindow () const
{
  NS_LOG_FUNCTION (this);
  return m_timeWindow;
}
...
// UE의 Age 값을 기반으로 우선순위를 결정
std::function<bool(const NrMacSchedulerNs3::UePtrAndBufferReq &lhs,
                   const NrMacSchedulerNs3::UePtrAndBufferReq &rhs )>
NrMacSchedulerOfdmaPF::GetUeCompareDlFn () const
{
  return [](const NrMacSchedulerNs3::UePtrAndBufferReq &lhs, const NrMacSchedulerNs3::UePtrAndBufferReq &rhs) {
    auto lhsUePtr = std::dynamic_pointer_cast<NrMacSchedulerUeInfoPF>(lhs.first);
    auto rhsUePtr = std::dynamic_pointer_cast<NrMacSchedulerUeInfoPF>(rhs.first);

    // PF 메트릭을 가져옴
    double lhsMetric = lhsUePtr->GetDlPfMetric();
    double rhsMetric = rhsUePtr->GetDlPfMetric();

    // Age 값을 가져옴
    uint64_t lhsAge = lhsUePtr->GetAge();
    uint64_t rhsAge = rhsUePtr->GetAge();

    // Age가 높은 쪽이 우선되도록 비교
    double lhsPriority = lhsMetric * lhsAge;
    double rhsPriority = rhsMetric * rhsAge;
...
NrMacSchedulerOfdmaPF::GetUeCompareUlFn () const
{
  return [](const NrMacSchedulerNs3::UePtrAndBufferReq &lhs, const NrMacSchedulerNs3::UePtrAndBufferReq &rhs) {
    ...
    // PF 메트릭과 Age 값을 기반으로 우선순위 비교
    double lhsMetric = lhsUePtr->GetUlPfMetric();
    double rhsMetric = rhsUePtr->GetUlPfMetric();
...
void NrMacSchedulerOfdmaPF::AssignedDlResources (const UePtrAndBufferReq &ue,
...
void NrMacSchedulerOfdmaPF::NotAssignedDlResources (const NrMacSchedulerNs3::UePtrAndBufferReq &ue,
...
void NrMacSchedulerOfdmaPF::AssignedUlResources (const UePtrAndBufferReq &ue,
                                            [[maybe_unused]] const FTResources &assigned,
                                                const FTResources &totAssigned) const
{
  NS_LOG_FUNCTION (this);
...
void NrMacSchedulerOfdmaPF::NotAssignedUlResources (const NrMacSchedulerNs3::UePtrAndBufferReq &ue,
                                               [[maybe_unused]] const NrMacSchedulerNs3::FTResources &assigned,
                                                   const NrMacSchedulerNs3::FTResources &totAssigned) const
{
  NS_LOG_FUNCTION (this);
...
void NrMacSchedulerOfdmaPF::BeforeDlSched (const UePtrAndBufferReq &ue,
...
void NrMacSchedulerOfdmaPF::BeforeUlSched (const UePtrAndBufferReq &ue,
                                          const FTResources &assignableInIteration) const
{
  NS_LOG_FUNCTION (this);
  auto uePtr = std::dynamic_pointer_cast<NrMacSchedulerUeInfoPF> (ue.first);
