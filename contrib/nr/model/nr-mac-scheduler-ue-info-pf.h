...
public:
  ...
  // 추가: DL/UL PF Metric 값을 반환하는 메서드
  double GetDlPfMetric() const { return m_potentialTputDl / std::max (1E-9, m_avgTputDl); }
  double GetUlPfMetric() const { return m_potentialTputUl / std::max (1E-9, m_avgTputUl); }
  // 추가: Age 설정 및 반환
  void SetAge (uint64_t age) { m_age = age; }
  uint64_t GetAge () const { return m_age; }
                                  ...
                                  const NrMacSchedulerNs3::UePtrAndBufferReq &rue)
  {
    ...
    // Age 기반 우선순위 비교
    if (luePtr->m_age != ruePtr->m_age)
    {
      return luePtr->m_age > ruePtr->m_age; // Age가 큰 UE를 우선 선택
    }
  ...
  uint64_t m_age; // 추가: 패킷의 Age 값 저장
};
